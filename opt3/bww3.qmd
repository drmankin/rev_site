...
ans2.2 <- round(round(summary(model2)$coefficients[2], 2) + 1.96*round(summary(model2)$coefficients[2,2], 2), 2) |> make_vec()

ans3 <- 2


lat <- paste0(ans3, ans2.1[1]) |> as.numeric()
...
- The second number is the first digit of the upper bound from task 2.

### Longitude:

- The first number is the third digit of the difference in R^2^ between the two models from task 1.
- The third number is the second digit of the lower bound from task 2.

When you have found the ship, click on the marker to proceed.

```{=html}
<iframe width="100%" height="650" src="https://drmankin.shinyapps.io/trackr/"></iframe>
```
